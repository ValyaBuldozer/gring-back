...
def localize_one_id_table(conn, table, id_col_name, text_col_names):
...
def localize_two_id_table(conn, table, col_id_names, text_col_names):
    table_name = str(table.fullname)
...
def localize_link(conn, table, col_id_names, text_col_names):
    table_name = str(table.fullname)
    id_names = ', '.join(col_id_names)
    joined_columns = ', '.join(text_col_names)
    query = 'select {0}, {1} from {2}'.format(id_names, joined_columns, table_name)
